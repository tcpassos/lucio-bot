    ...
    public void execute(SlashCommandInteractionEvent event) {
        ...
        if (utils.channelControl(event)) {
            ...
            if (previousVolume < volume) {
                playerManagerService.loadAndPlaySfx(event.getGuild(), sfxService.getSoundFor(SfxType.VOLUME_UP));
            }
            if (previousVolume > volume && volume < 30) {
                playerManagerService.loadAndPlaySfx(event.getGuild(), sfxService.getSoundFor(SfxType.VOLUME_LOW));
            }
